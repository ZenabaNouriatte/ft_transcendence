...
async function forward(req: any, reply: any, prefix: string, target: string) {
  const subpath = req.url.startsWith(prefix) ? req.url.slice(prefix.length) : "";
  ...
  // utilise le fetch global (Node >=18)
